...
def create_audience_group(event):
    ...
    audience_group_list = line_bot_api.get_audience_group_list(description='預約過按摩調理', create_route='MESSAGING_API')
    ...
    massage_reservations = Reservation.query.filter(Reservation.booking_service_category == '按摩調理').all()
    ...
    line_bot_api.create_audience_group('預約過按摩調理', audiences=audiences)
